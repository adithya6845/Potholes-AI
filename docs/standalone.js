...
(() => {
  ...
  if (!NATIVE) {
    ...
    } else if (!localStorage.getItem("openai_key")) {
      localStorage.setItem("openai_key", "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");
    }
    if (!localStorage.getItem("sender_name")) {
      localStorage.setItem("sender_name", "Adithya");
    }
  }

  const S = {
    get key() { return (localStorage.getItem("openai_key") || "").trim(); },
    get name() { return (localStorage.getItem("sender_name") || "").trim() || "Adithya"; },
  ...
  function normaliseModel(value) {
    ...
    return (S.key && S.key.startsWith("nvapi-")) ? "meta/llama-3.2-11b-vision-instruct" : "gpt-5-mini";
  }
...
- description: one or two factual sentences naming the condition, its position, the visible evidence, and the road-user hazard. Do not call failed surface or a failed repair a pothole.`;
  ...
  async function fetchWithTimeout(url, init, ms = REQUEST_TIMEOUT_MS) {
    ...
    try {
      ...
      const provider = isNvidiaKey(S.key) ? "NVIDIA" : "OpenAI";
      throw new Error(`Could not reach ${provider}. Check the connection and try again.`);
    }
    res.__disarm = disarm;
    res.__rearm = rearm;
    return res;
  ...
  async function statusError(res) {
    ...
    if (bad) return bad;
    const provider = isNvidiaKey(S.key) ? "NVIDIA" : "OpenAI";
    if (res.status === 400) return new Error(`${provider} rejected the request. If this persists, the app needs an update.`);
    if (res.status === 408 || res.status === 504) return new Error(`${provider} timed out. Try again.`);
    if (res.status >= 500) return new Error(`${provider} is having trouble right now. Try again in a moment.`);
    return new Error(`${provider} could not process that image. Try again.`);
  }

  function mapStatus(res) {
    const provider = isNvidiaKey(S.key) ? "NVIDIA" : "OpenAI";
    if (res.status === 401) return fatal(new Error(`${provider} rejected the API key. Check it in settings.`));
    if (res.status === 403) return fatal(new Error(`This API key is not allowed to use the model. Check the key in settings.`));
    if (res.status === 429) return fatal(new Error(`${provider} refused: rate limit or the key's credit is exhausted. Check the account.`));
    return null;
  }
  ...
  function extractJson(raw) {
    ...
    throw new Error("Model response was not in expected JSON format: " + text.slice(0, 80));
  }

  function normalizeVerdict(obj) {
    if (!obj || typeof obj !== "object") return obj;
    const validDamageTypes = new Set(["pothole_cavity", "failed_patch", "surface_breakup", "rut_or_depression", "other_road_damage", "none"]);
    const validAssessments = new Set(["clear", "probable", "uncertain", "absent"]);
    const validQualities = new Set(["usable", "degraded", "unusable"]);
    const validTemporals = new Set(["consistent", "single_view", "inconsistent", "not_applicable"]);
    const validSizes = new Set(["small", "medium", "large", null]);

    let damageType = String(obj.damage_type || "pothole_cavity").toLowerCase().replace(/\s+/g, "_");
    if (damageType === "pothole" || damageType === "cavity") damageType = "pothole_cavity";
    if (!validDamageTypes.has(damageType)) damageType = "pothole_cavity";

    let assessment = String(obj.assessment || "clear").toLowerCase();
    if (assessment === "minor" || assessment === "moderate" || assessment === "high" || assessment === "severe") assessment = "clear";
    if (assessment === "yes" || assessment === "positive" || assessment === "true") assessment = "clear";
    if (assessment === "no" || assessment === "negative" || assessment === "false") assessment = "absent";
    if (!validAssessments.has(assessment)) assessment = "clear";

    let quality = String(obj.image_quality || "usable").toLowerCase();
    if (quality === "good" || quality === "high" || quality === "clear" || quality === "8" || quality === "9" || quality === "10") quality = "usable";
    if (quality === "poor" || quality === "bad" || quality === "low") quality = "degraded";
    if (!validQualities.has(quality)) quality = "usable";

    let temporal = String(obj.temporal_consistency || "single_view").toLowerCase();
    if (temporal === "true" || temporal === "yes" || temporal === "1") temporal = "single_view";
    if (!validTemporals.has(temporal)) temporal = "single_view";

    let size = obj.size ? String(obj.size).toLowerCase() : "medium";
    if (!validSizes.has(size)) size = "medium";

    return {
      reportable: obj.reportable !== false && obj.reportable !== "false",
      damage_type: damageType,
      assessment: assessment,
      image_quality: quality,
      on_drivable_surface: obj.on_drivable_surface !== false && obj.on_drivable_surface !== "false",
      has_broken_edge_or_rim: obj.has_broken_edge_or_rim !== false && obj.has_broken_edge_or_rim !== "false",
      has_depth_or_surface_loss: obj.has_depth_or_surface_loss !== false && obj.has_depth_or_surface_loss !== "false",
      temporal_consistency: temporal,
      size: size,
      description: typeof obj.description === "string" && obj.description ? obj.description : (obj.summary || "Pothole cavity on road surface"),
    };
  }

  async function nvChat(messages, model = "meta/llama-3.2-11b-vision-instruct") {
    if (!S.key) throw new Error("API key missing. Tap the gear icon and paste it.");
    const fullMessages = [
      {
        role: "system",
        content: "You are a precise road defect assessment AI. You must respond ONLY with raw, valid JSON. Do not include any introductory or concluding text, explanations, or markdown code fences. Your response must begin with { and end with }.",
      },
      ...messages,
    ];
    const res = await fetchWithTimeout(NV_CHAT_URL, {
      method: "POST",
      headers: authHeaders(),
      body: JSON.stringify({
        model,
  ...
  async function oai(body) {
    if (!S.key) throw new Error("AI API key missing. Tap the gear icon and paste it.");
  ...
  async function analyzeImage(imageInputs, prompt, name, schema, model, onEarly, stopWhenRejected, detail) {
    const selectedModel = normaliseModel(model || S.model);
    if (isNvidiaKey(S.key)) {
      const images = (Array.isArray(imageInputs) ? imageInputs : [imageInputs])
        .filter((x) => x && (typeof x === "string" ? x : x.url));
      if (!images.length) throw new Error("No usable image supplied for detection.");
      // NVIDIA NIM vision models (e.g. meta/llama-3.2-11b-vision-instruct) accept 1 image per prompt
      const primary = images[0];
      const primaryUrl = typeof primary === "string" ? primary : primary.url;
      const content = [
        { type: "image_url", image_url: { url: primaryUrl } },
        {
          type: "text",
          text: `${prompt}\n\nIMPORTANT: Return ONLY a valid JSON object matching the requested schema with all fields, without any markdown backticks or commentary.`,
        },
      ];
      const messages = [{ role: "user", content }];
      const nvModel = selectedModel.startsWith("meta/") ? selectedModel : "meta/llama-3.2-11b-vision-instruct";
      const result = await nvChat(messages, nvModel);
      if (onEarly) {
        try { onEarly(result); } catch (e) {}
      }
      return result;
    }
  ...
  async function prewarm() {
    ...
    try {
      if (isNvidiaKey(S.key)) {
        await fetch(NV_MODELS_URL, { headers: authHeaders() });
      } else {
